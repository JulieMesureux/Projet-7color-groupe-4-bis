...
void GP4_b_play(GameState* state, Color player, Color coup, element** front){
    ...
    if (coup == EMPTY){ 
        ...
        GP4_b_MAJ_plateau(state, player , front, Couleur) ;
    }
    else{
        //printf("Au tour du joueur %d\n\n", player) ;

        GP4_b_MAJ_plateau(state, player, front , coup) ;
