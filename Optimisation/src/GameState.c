...
int main(int argc, char** argv){
	...
	// Création de la liste des frontières des joueurs
	...
	if  (nbr_joueur == 2){    // Partie classique à deux joueurs
		GP4_b_two_players_game(&state, &front_1, &front_2) ;
	}
		...
		if (argc == 3){  // On n'a pas précisé l'IA
			...
			GP4_b_one_player_game(&state, IA, &front_1, &front_2) ;
		}
		...
		if (argc <= 4){  // On n'a pas précisé les deux IA
			...
			GP4_b_AI_game(&state, IA_1, IA_2, &front_1, &front_2) ;
		}
			...
			GP4_b_competition(&state, IA_1, IA_2, nbr_parties, &front_1, &front_2) ;
		}
	}

	GP4_b_supprime_liste(&front_1) ;
	GP4_b_supprime_liste(&front_2) ;
